...
// @desc    Remove an email from the newsletter
...
const unsubscribeNewsletter = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ message: 'You have been removed from the newsletter list.' });
  } catch (error) {
    next(error);
  }
};
...
// @desc    Delete a newsletter subscriber from admin panel
